...
# --- Fitness Function (FIXED Finally Block Structure) ---
def fitness_func(ga_instance, solution, solution_idx):
    ...
    try:
        ...
        try:
            ...
            if vessel_sit == VS.pre_launch or vessel_sit == VS.landed or vessel_sit == VS.splashed:
        ...
        except (krpc.error.RPCError, RuntimeError, AttributeError, TypeError) as setup_e: print(f"!! FATAL ERROR during evaluation SETUP ({type(setup_e).__name__}): {setup_e}"); import traceback; traceback.print_exc();
        ...
        if setup_successful:
            # (Simulation loop code remains the same as previous version)
            ...
            # (Cleanup and Fitness Calc code remains the same as previous version)
            print("  Simulation loop processing finished.")
            try:
                 print("  Resetting yaw control after loop...")
    ...
    # <<< Corrected Finally Block Structure >>>
    finally:
        # This block runs regardless of errors in the main try block
        if conn: # Only proceed if connection was potentially established
            print("  Cleaning up controls and connection after evaluation...")
            # Try to reset controls
            try:
                if 'control' in locals() and control is not None:
                    try: control.yaw = 0
                    except krpc.error.RPCError: pass # Ignore RPC errors resetting yaw
                    if PITCH_CONTROL_METHOD == 'SAS_STABILITY':
                        try: control.sas = False
                        except krpc.error.RPCError: pass # Ignore RPC errors disabling SAS
                if 'ap' in locals() and ap is not None and PITCH_CONTROL_METHOD == 'AP_PITCH':
                    try: ap.disengage()
                    except (krpc.error.RPCError, AttributeError): pass # Ignore RPC/Attr errors disengaging AP
            except Exception as e_clean:
                # Catch errors during the cleanup *itself*
                print(f"  Unexpected error during control cleanup: {e_clean}")
            ...
            try:
                ...
                print("  Error closing kRPC connection (might already be closed).")
                pass # Ignore errors closing connection

    print(f"  Fitness returned for Sol {solution_idx} (Gen {gen_num}): {fitness:.4f}")
    return fitness

# --- The rest of the script (Callback, GA Setup, Run, Results) remains unchanged ---
        ...
        try:
            ...
            try:
                ...
                if control.sas:
                    ...
                    try:
                        if hasattr(sc, 'SASMode'): ap.sas_mode = sc.SASMode.stability_assist; print("    SAS Mode set to Stability Assist.")
            ...
            except krpc.error.RPCError as sas_set_e: print(f"!! ERROR: kRPC Error attempting to set control.sas: {sas_set_e}")
...
# Using parameters from top of script
...
print("Ensure KSP is running and the vessel is in a controllable state (FLYING!).")
